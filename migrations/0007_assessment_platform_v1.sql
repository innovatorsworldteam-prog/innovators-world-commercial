...
-- Migration 0007: canonical assessment-platform foundation
-- Additive only. Existing IWDA production tables/data are preserved.
...
CREATE INDEX IF NOT EXISTS idx_assessment_dimensions_version ON assessment_dimensions(assessment_version_id);
...
CREATE INDEX IF NOT EXISTS idx_assessment_questions_version_order ON assessment_questions(assessment_version_id, display_order);
...
CREATE INDEX IF NOT EXISTS idx_assessment_options_question_order ON assessment_options(question_id, display_order);
...
CREATE INDEX IF NOT EXISTS idx_assessment_attempts_participant ON assessment_attempts(participant_id, assessment_id, status);
CREATE INDEX IF NOT EXISTS idx_assessment_attempts_anonymous ON assessment_attempts(anonymous_session_id, assessment_id, status);
CREATE INDEX IF NOT EXISTS idx_assessment_attempts_version ON assessment_attempts(assessment_version_id);
...
CREATE TABLE IF NOT EXISTS assessment_results (
  ...
  participant_id TEXT,
  user_id TEXT,
  result_status TEXT NOT NULL DEFAULT 'complete' CHECK (result_status IN ('complete','invalidated')),
  overall_score REAL,
...
CREATE INDEX IF NOT EXISTS idx_assessment_results_participant ON assessment_results(participant_id, assessment_id, generated_at);
...
CREATE INDEX IF NOT EXISTS idx_assessment_dimension_scores_result ON assessment_dimension_scores(result_id);

INSERT OR IGNORE INTO assessments (id, code, name, description, status, created_at, updated_at) VALUES
('assessment-iwda','IWDA','Innovators World Discovery Assessment','Assessment of innovation-thinking dimensions. Existing IWDA scoring remains canonical.','active',datetime('now'),datetime('now')),
('assessment-career-discovery','CAREER_DISCOVERY','Innovators World Career Discovery','Career-exploration assessment covering interests, activities, values, environments, skill inclinations and future curiosity.','draft',datetime('now'),datetime('now'));

INSERT OR IGNORE INTO assessment_versions (id, assessment_id, version, status, methodology_version, scoring_version, matching_version, created_at, updated_at) VALUES
...
INSERT OR IGNORE INTO assessment_dimensions (id, assessment_version_id, code, name, description, display_order, scoring_weight, created_at, updated_at) VALUES
