...
type ScoredOption = AssessmentOption & { score: number; dimension: string; careerWorldSignals: string[] };
...
function questionDimension(question: AssessmentQuestion): string | null {
  const dimension = parseJson(question.scoring_config_json).dimension;
...
function optionScore(option: AssessmentOption, expectedDimension: string): ScoredOption | null {
  ...
  const signals = Array.isArray(metadata.career_world_signals) ? metadata.career_world_signals.filter((item): item is string => typeof item === "string") : [];
  return { ...option, score: raw, dimension: expectedDimension, careerWorldSignals: signals };
}

export async function careerDiscoveryQuestions(db: D1Database, age: number): Promise<{ version: AssessmentVersion; questions: Array<AssessmentQuestion & { options: AssessmentOption[] }> }> {
  ...
  const hydrated = await Promise.all(questions.map(async (question) => ({ ...question, options: await listQuestionOptions(db, question.id) })));
  return { version, questions: hydrated };
}

export async function startCareerDiscovery(db: D1Database, input: { age: number; participantId?: string | null; userId?: string | null; anonymousSessionId?: string | null; tier?: string | null }) {
  ...
  const existing = await db.prepare(`SELECT id FROM assessment_attempts WHERE assessment_id = ? AND ${ownerColumn} = ? AND status = 'started' ORDER BY started_at DESC LIMIT 1`).bind(assessment.id, owner).first<{ id: string }>();
  if (existing) return { attempt: await getAttempt(db, existing.id), resumed: true, tier };
  const attempt = await createAttempt(db, {
    id: crypto.randomUUID(), assessmentId: assessment.id, assessmentVersionId: version.id,
    participantId: input.participantId ?? null, userId: input.userId ?? null, anonymousSessionId: input.anonymousSessionId ?? null,
    metadata: { product: CAREER_DISCOVERY_CODE, tier, age: input.age, algorithm_version: CAREER_DISCOVERY_ALGORITHM },
  });
  return { attempt, resumed: false, tier };
}

export async function recordCareerDiscoveryAnswer(db: D1Database, input: { attemptId: string; questionId: string; optionId: string; answerValue?: string | null }): Promise<void> {
  ...
  const question = await db.prepare(`SELECT * FROM assessment_questions WHERE id = ? AND assessment_version_id = ? LIMIT 1`).bind(input.questionId, attempt.assessment_version_id).first<AssessmentQuestion>();
  ...
  await upsertAnswer(db, { id: crypto.randomUUID(), attemptId: input.attemptId, questionId: input.questionId, optionId: input.optionId, answerValue: input.answerValue ?? selected.code });
}

export async function calculateCareerDiscoveryResult(db: D1Database, attemptId: string) {
  ...
  for (const question of questions) {
    ...
    current.total += scored.score; current.max += 4; current.count += 1; dimensionScores.set(dimension, current);
  ...
  const resultDimensions = dimensions.map((dimension: AssessmentDimension) => {
    ...
    return { dimension_id: dimension.id, code: dimension.code, name: dimension.name, raw_score: data.total, max_score: data.max, normalized_score: Math.max(0, Math.min(100, normalized)), weight: dimension.scoring_weight, interpretation_band: normalized >= 75 ? "strong" : normalized >= 50 ? "moderate" : "emerging" };
  });
  const overall = Math.round(resultDimensions.reduce((sum, item) => sum + item.normalized_score * item.weight, 0) * 100) / 100;
  const topSignals = [...careerSignals.entries()].sort((a, b) => b[1] - a[1]).slice(0, 8).map(([code, signalScore]) => ({ code, signal_score: signalScore }));
  return { algorithm_version: CAREER_DISCOVERY_ALGORITHM, assessment: CAREER_DISCOVERY_CODE, version_id: attempt.assessment_version_id, overall_score: overall, dimensions: resultDimensions, career_world_signals: topSignals, evidence, interpretation_notice: "Career Discovery identifies exploration signals and does not prescribe or guarantee a career outcome." };
}

export async function completeCareerDiscovery(db: D1Database, attemptId: string) {
  const existing = await db.prepare(`SELECT * FROM assessment_results WHERE attempt_id = ? AND result_status = 'complete' LIMIT 1`).bind(attemptId).first<{ id: string; attempt_id: string; assessment_id: string; assessment_version_id: string; participant_id: string | null; user_id: string | null; result_status: "complete" | "invalidated"; overall_score: number | null; result_json: string; algorithm_version: string | null; generated_at: string }>();
  if (existing) return { result: existing, reused: true };
  const attempt = await getAttempt(db, attemptId);
  if (!attempt) throw new Error("ASSESSMENT_ATTEMPT_NOT_FOUND");
  if (attempt.status !== "started") throw new Error("ASSESSMENT_ATTEMPT_NOT_ACTIVE");
  const calculated = await calculateCareerDiscoveryResult(db, attemptId);
  const result = await saveResult(db, { id: crypto.randomUUID(), attemptId, assessmentId: attempt.assessment_id, assessmentVersionId: attempt.assessment_version_id, participantId: attempt.participant_id, userId: attempt.user_id, overallScore: calculated.overall_score, result: calculated, algorithmVersion: CAREER_DISCOVERY_ALGORITHM });
  await saveDimensionScores(db, result.id, calculated.dimensions.map((dimension) => ({ id: crypto.randomUUID(), dimensionId: dimension.dimension_id, rawScore: dimension.raw_score, normalizedScore: dimension.normalized_score, maxScore: dimension.max_score, interpretation: dimension.interpretation_band, metadata: { weight: dimension.weight } })));
  await db.prepare(`UPDATE assessment_attempts SET status='completed', completed_at=? WHERE id=? AND status='started'`).bind(new Date().toISOString(), attemptId).run();
