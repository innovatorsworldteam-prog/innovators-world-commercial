...
/*
 * Calculate the true maximum possible score for every dimension
 * from the current IWDA question bank.
 *
 * This avoids assuming that every dimension has the same maximum.
 */
function calculateDimensionMaximums(): Record<IwdaDimension, number> {
  ...
  for (const question of IWDA_QUESTIONS) {
    ...
    const primaryMaximum = Math.max(
      ...options.map((option) => option.primary)
    );

    const secondaryMaximum = Math.max(
      ...options.map((option) => option.secondary)
    );
...
export function calculateIWDAResult(
  ...
  const dimensionScores: IWDADimensionScore[] =
    IWDA_DIMENSIONS.map((dimension) => {
  ...
  const ranked = [...dimensionScores].sort((a, b) => {
    ...
    return (
      IWDA_DIMENSIONS.indexOf(a.dimension) -
      IWDA_DIMENSIONS.indexOf(b.dimension)
    );
  });

  const primaryStrength =
    ranked[0]?.dimension ?? IWDA_DIMENSIONS[0];

  const secondaryStrength =
    ranked[1]?.dimension ?? primaryStrength;

  const growthDimension =
    ranked[ranked.length - 1]?.dimension ??
    IWDA_DIMENSIONS[IWDA_DIMENSIONS.length - 1];
  ...
  return {
    ...
    result_data: {
      ...
      innovation_readiness_index:
        innovationReadinessIndex,
      dimensions,
      primary_strength: {
        code: primaryStrength,
        name: DIMENSION_NAMES[primaryStrength],
      },
      secondary_strength: {
        code: secondaryStrength,
        name: DIMENSION_NAMES[secondaryStrength],
      },
      growth_dimension: {
        code: growthDimension,
        name: DIMENSION_NAMES[growthDimension],
      },
      traits,
